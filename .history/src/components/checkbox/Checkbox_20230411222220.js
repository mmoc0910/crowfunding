    ...
    <div className="flex items-start gap-5">
      <div
        className={`w-5 h-5 rounded-[4px] border flex items-center justify-center transition-all duration-75 ${
          checked
            ? "border-primary bg-primary dark:border-text3 text-white"
            : "border-text4 bg-white dark:bg-dark-secondary dark:border-text3"
        }`}
        onClick={onClick}
      >
        <input type="checkbox" className="hidden" />
        <span className="text-white dark:text-dark-secondary">
          <IconCheck />
        </span>
      </div>
      {children && (

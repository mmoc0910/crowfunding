...
const DashboardSearch = () => {
  ...
  React.useEffect(() => {
    async function fetchSearchCampaign() {
      if (search) {
        const response = await axiosPrivate.get(
          `api/campaigns/?_limit=2&title_like=${search}`
        );
  ...
  return (
    <div className="relative z-50 max-w-[217px] md:max-w-[458px] w-full mx-auto xl:mx-0">
      <div
        className="w-full"
        onClick={() => !isFocus && dispatch(setFocus(true))}
      >
        <div className="w-full p-[6px] pl-[15px] bg-white dark:bg-dark-secondary rounded-[100px] dark:shadow-none shadow-[10px_10px_20px_rgba(218,213,213,0.15)] flex items-center min-h-[40px] h-full md:min-h-[52px]">
          <div className="flex-1">
            <input
              ...
              onKeyUp={debounce(handleOnKeyUp, 200)}
            />
          </div>
      ...
      <div
        className={classNames(
          "w-[calc(100vw-50px)] md:w-[calc(70vw)] absolute -translate-x-[calc(50%-12px)] lg:translate-x-0 left-1/2 lg:left-0 top-full mt-5 bg-white rounded-[20px] lg:w-[700px] xl:w-[820px] dark:bg-dark-secondary",
          isFocus ? "" : "hidden"
        )}
      >
        <div className="p-[10px] flex items-center justify-between dark:bg-dark-soft dark:rounded-[20px]">
          <h4 className="text-xs md:text-sm font-medium p-[10px] underline dark:text-white text-text1">
            {search.length <= 0
              ? "Did not enter the search results"
              : data.length <= 0
              ? "No matching results"
              : `See all ${data.length} fundraisier`}
          </h4>
          <button
            className="cursor-pointer w-[48px] h-[34px] md:w-[72px] md:h-[50px] flex items-center justify-center dark:bg-[#422C32] bg-[#F9E3E3] rounded-[10px]"
            onClick={() => dispatch(setFocus(false))}
          >
            <IconClose />
          </button>
        </div>
        <div className="pt-[10px] pb-[25px] px-[15px] md:p-[25px]">
          {search && data.length > 0 && (
            <div className="space-y-[15px] md:space-y-6 mb-[30px]">
              {data.map((item) => (
                <SearchItem key={v4()} campaign={item} />
              ))}
            </div>
          )}
...
const SearchItem = ({ campaign }) => {
  return (
    <Link
      reloadDocument
      to={`/campaign/${campaign.slug}`}
      className="flex items-center gap-[15px] md:gap-5"
    >
      <img
        src={campaign.images[0] || defaultImg}
        alt={campaign.title}
      ...
      <div className="flex-1">
        ...
        <h4 className="mb-1 text-xs md:text-sm text-text1 dark:text-white line-clamp-1">
          {campaign.title}
        </h4>

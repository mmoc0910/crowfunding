      ...
      <div className="w-full">
        <div className="w-full p-[6px] pl-[15px] bg-white dark:bg-dark-secondary rounded-[100px] dark:shadow-none shadow-[10px_10px_20px_rgba(218,213,213,0.15)] flex items-center min-h-[40px] h-full md:min-h-[52px]">
          <div className="flex-1">
            <input
              type="text"
              className="text-xs md:text-sm placeholder:text-text4 p-[10px] w-full text-text1 bg-inherit dark:placeholder:text-text2 dark:text-white"
              placeholder="Do fundrise now"
            />
          </div>

          <button className="bg-primary dark:bg-primary20 rounded-[20px] md:px-[27px] md:py-[11px] px-[14px] py-[7px] flex-shrink-0 ">
            <IconSearch />
          </button>
        </div>
      </div>
      <div className="absolute hidden left-0 top-full mt-5 bg-white rounded-[20px] w-full md:w-[600px] lg:w-[843px] dark:bg-dark-secondary">
        <div className="p-[10px] flex items-center justify-between dark:bg-dark-soft dark:rounded-[20px]">
          <h4 className="text-sm font-medium p-[10px] underline dark:text-white text-text1">
            See all 10,124 fundraisier
          </h4>
          <button className="cursor-pointer w-[72px] h-[50px] flex items-center justify-center dark:bg-[#422C32] bg-[#F9E3E3] rounded-[10px]">
            <IconClose />
          </button>
        </div>
...
const SearchItem = () => {
  return (
    <Link className="flex items-center gap-5">
      ...
      />
      <div className="flex-1">
        <h4 className="mb-1 text-sm text-text1 dark:text-white">

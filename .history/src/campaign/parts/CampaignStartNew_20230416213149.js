...
const CampaignStartNew = () => {
  ...
  const [fillterCountry, setFillterValue] = useOnchange();
  ...
  return (
    <form className="py-10 pr-16 pl-28 dark:bg-dark-secondary rounded-[10px]">
            ...
            <Dropdown>
              ...
              <Dropdown.List>
                <Dropdown.Search
                  ...
                  onChange={setFillterValue}
                ></Dropdown.Search>
                {countries.length > 0 && countries.map(country => <Dropdown.Option key={}>{country.nam.common}</Dropdown.Option>)}
                {/* 
                <Dropdown.Option>Tablet Games</Dropdown.Option>
                <Dropdown.Option>Video Games</Dropdown.Option> */}
              </Dropdown.List>
      ...
      <Button className="px-10 mx-auto mt-10 text-white bg-primary">
        Submit new campaign{" "}
      </Button>
    </form>
  );
